  ...
  const check = async (entityId: string, pointers: string[], promise: Response) => {
    if (!promise.ok) {
      ...
      if (failManifest.ok) {
        ...
        console.log(
          `🟠 ${entityId} (${pointers[0]}): Failed. Version=${manifest.version} ExitCode=${manifest.exitCode} Date=${manifest.date} Log=${manifest.log}`
        )
      } else {
      ...
      console.log(
        `🟢 ${entityId} (${pointers[0]}): Version=${manifest.version} ExitCode=${manifest.exitCode} Date=${manifest.date}`
      )
    }
  }
